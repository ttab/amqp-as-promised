...
(function() {
  ...
  module.exports = function(conf) {
    var bind, conn, exchange, isShutdown, queue, shutdown, shutdownDef, unbind, wrapQueue;
    if (conf.local) {
    ...
    queue = function(qname, opts) {
      ...
      conn.then(function(mq) {
        ...
        return mq.queue(qname, opts, function(queue) {
          ...
          return def.resolve(wrapQueue(queue));
        });
      }).done();
      return def.promise;
    };
    wrapQueue = function(q) {
      var qname;
      qname = q.name;
      return {
        bind: function(ex, topic) {
          var def;
          def = Q.defer();
    ...
    bind = function(exname, qname, topic, callback) {
      ...
      (Q.all([exchange(exname), queue(qname)])).spread(function(ex, q) {
        return (q.bind(ex, topic)).then(function() {
          return def.resolve(ex);
        });
      }).done();
      return def.promise;
